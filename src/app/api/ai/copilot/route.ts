...
async function handleRequest(req: NextRequest, body?: any) {
  ...
  const model = (body?.model ?? req.nextUrl.searchParams.get('model'))?.toString() ?? 'default';
  ...
  try {
    ...
    return NextResponse.json({ status: false, creator: siteConfig.api.creator, message: 'Failed to get response from Copilot.', error: err?.message || String(err) }, { status: 500 });
  }
}
